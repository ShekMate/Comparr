...
export async function initPosterCache(): Promise<void> {
  try {
    ...
    try {
      ...
      log.info(`📦 Loaded poster cache: ${Object.keys(cacheMetadata.entries).length} posters, ${(cacheMetadata.totalSize / 1024 / 1024).toFixed(2)}MB`)
    } catch {
...
async function evictIfNeeded(): Promise<void> {
  ...
  log.info(`🗑️ Cache size ${(cacheMetadata.totalSize / 1024 / 1024).toFixed(2)}MB exceeds limit, evicting old posters...`)
  ...
  log.info(`✅ Evicted ${evictedCount} posters, freed ${(freedSpace / 1024 / 1024).toFixed(2)}MB`)
}
...
export async function cachePoster(
  ...
  try {
    ...
    log.debug(`✅ Cached poster: ${filename} (${(imageData.length / 1024).toFixed(2)}KB)`)
    return `/cached-poster/${filename}`
  } catch (err) {
 ...
 */
export async function serveCachedPoster(filename: string, req: any): Promise<boolean> {
  const filepath = `${POSTER_CACHE_DIR}/${filename}`
...
export function getBestPosterUrl(posterPath: string, source: 'plex' | 'tmdb'): string {
