...
async function maybeRunUserOnboardingWizard(currentUser) {
  ...
  await new Promise(resolve => {
    ...
    modal.innerHTML = `
      <div class="first-run-guide-content">
        <h3 id="user-onboarding-title">Welcome to Comparr</h3>
        <p id="user-onboarding-copy" class="first-run-guide-instruction"></p>
...
function createFirstRunGuideModal() {
  ...
  const renderScreen = screen => {
    ...
    if (screen.type === 'user-auth') {
      ...
      body.innerHTML = `
        <p class="first-run-guide-instruction">
          Plex authentication is always enabled. Every user must sign in with Plex before using Comparr.
        </p>
        ${plexRestrictRow}
        <p id="first-run-user-auth-note" class="first-run-guide-instruction">
